...
class FileUploader {
    ...
    async validateFileType(file, allowedTypes) {
        ...
        if (!allowedTypes.includes(file.mimetype)) {
            return { error: `Invalid file type: ${file.mimetype}` };
        }
    ...
    async validateFileSize(file, options) {
        ...
        if (file.size > options.maxSize) {
            return { error: `File size exceeds the maximum limit of ${options.maxSize} bytes` };
        }
    ...
    async validateImage(file, options) {
        ...
        try {
            ...
            } else if (!options.cropImage && (metadata.width > options.maxWidth || metadata.height > options.maxHeight)) {
                return { error: 'Image dimensions exceed maximum allowed dimensions. CropImage option is false. Image cannot be uploaded.' };
            } else {
                log(2, 'Image dimensions within maximum allowed dimensions. No need for cropping');
            }
    ...
    async validate(filesToValidate) {
            ...
            for (const file of files) {
                ...
                if (fieldOptions.type === 'image') {
                    ...
                    if (imageValidation.error) {
                        validationErrors.push(imageValidation);
                    }
                } else if (fieldOptions.type === 'audio') {
                    const audioValidation = await this.validateAudio(file, fieldOptions);
                    if (audioValidation.error) {
                        validationErrors.push(audioValidation);
                    }
                }
        ...
        if (requiresSongs && !filesToValidate['songs']) {
            validationErrors.push({ error: 'At least one song must be uploaded for the album' });
        }
